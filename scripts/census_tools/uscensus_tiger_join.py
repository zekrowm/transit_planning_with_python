...
# Optional FIPS filter (5-char county codes). Leave empty to keep all.
...
def _load_csv_to_memory(csv_path: str) -> str:
    ...
    AttributeError you hit when trying to call TableToTable from ``arcpy.management``.
    ...
    out_name = "attrs_csv"
    LOGGER.info(
        "Loading CSV → in_memory table: %s → %s",
        csv_path,
        out_name,
    )
