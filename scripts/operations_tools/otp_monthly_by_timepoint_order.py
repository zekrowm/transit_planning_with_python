...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description="Recalculate OTP percentages, apply optional filters, and output pivot+summary tables."
    )
...
def add_year_month_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in candidates:
        if c in df.columns:
            ...
            if (ym != "").any():
                ...
                if parsed.isna().all():
                    sys.exit(
                        f"ERROR: Found {c!r} but none of its values parse as 'YYYY-MM'. Fix the export."
                    )
    ...
    # Prefer year inferred from matching month number (Month column) where possible; else fall back to most common year.
    ...
    if df["Year-Month"].astype(str).eq("NaT").any():
        bad = df.loc[df["Year-Month"].astype(str).eq("NaT"), ["Month", "Route", "Direction", "Variation"]].head(10)
        sys.exit(
            "ERROR: Could not infer Year-Month for some rows (missing Month and missing Date-derived month). "
...
def filter_month_range(df: pd.DataFrame, start_month: str | None, end_month: str | None) -> pd.DataFrame:
    ...
    if dropped:
        logging.info("Month filter dropped %d rows outside [%s, %s].", dropped, start_month, end_month)
...
def build_timepoint_lookup_for_group(group_df: pd.DataFrame) -> pd.DataFrame:
    ...
    g = group_df[["Timepoint Order", "Timepoint ID", "Timepoint Description", "Total Counts"]].copy()
...
def save_variation_line_plot(summary_df: pd.DataFrame, outpath: Path, dpi: int, otp_standard: float) -> None:
...
def pivot_monthly_by_stop(
    ...
    for (route, direction, variation), g in df_monthly.groupby(group_cols):
        ...
        if not cfg_orders:
            logging.warning("[%s %s %s] No Timepoint Order values found; skipped.", route, direction, variation)
            continue
        ...
        if ctx.empty:
            ...
            if "Total Counts" not in ctx_tmp.columns:
                ...
                if "Sum # Processed" in ctx_tmp.columns:
                    ctx_tmp["Total Counts"] = pd.to_numeric(ctx_tmp["Sum # Processed"], errors="coerce").fillna(0)
                else:
...
def main() -> None:
    ...
    n_map = {
        (r, d, v): int(n)
        for r, d, v, n in variation_index[["Short Route", "Direction", "Variation", "N"]].itertuples(
    ...
    pivot_cnt = pivot_monthly_by_stop(df_monthly, "Total Counts", ascending=SORT_TIMEPOINT_ORDER_ASC)
    ...
    for (route, direction, variation), wide_pct in pivot_pct.items():
        ...
        if write_lookups:
            tp_lookup = summary_df[["Timepoint Order", "Timepoint ID", "Timepoint Description"]].copy()
            tp_lookup.to_csv(outdir / f"{stem}_timepoints.csv", index=False)

        if write_plots:
            plot_path = outdir / f"{stem}_otp_line.png"
            save_variation_line_plot(summary_df, plot_path, dpi=PLOT_DPI, otp_standard=OTP_STANDARD_PCT)

        logging.info("Wrote %s* files for %s %s %s (n=%d).", stem, route, direction, variation, n_total)
