...
def get_distance_conversion_factor(unit: str) -> float:
    ...
    if unit_lower == "meters":
        ...
        raise ValueError(
            f"Unknown distance unit={unit}. Must be 'meters','feet','miles'."
        )
...
def main():
    try:
        ...
        trip_metrics = stop_times_df.groupby("trip_id", as_index=False).apply(
            calculate_run_time_and_distance
        )
        # Attach route_id, direction_id
        trip_metrics = trip_metrics.merge(
            trips_df[["trip_id", "route_id", "direction_id"]], on="trip_id", how="left"
        )
        ...
        route_dir_metrics = trip_metrics.groupby(
        ...
        # Speed
        route_dir_metrics["avg_speed_mph"] = route_dir_metrics["avg_distance_miles"] / (
        ...
        for sched_type, days in schedule_types.items():
            ...
            first_stops = first_stops.merge(trip_info, on="trip_id", how="left")
            ...
            first_stops["time_block"] = first_stops["departure_time"].apply(
                block_assigner
            )

            # (e) HEADWAYS: group by (route_id, direction_id, time_block)
            headway_df = (
                first_stops.groupby(["route_id", "direction_id", "time_block"])[
                    "departure_time"
                ]
            ...
            route_lookup = routes_df[
                ["route_id", "route_short_name", "route_long_name"]
            ].drop_duplicates()
            final_data = final_data.merge(route_lookup, on="route_id", how="left")
            ...
            final_data["avg_run_time"] = final_data["avg_run_minutes"].apply(
                minutes_to_hhmm
            )
