...
def load_gtfs(folder: Path) -> Dict[str, pd.DataFrame]:
    """Read the core GTFS text files.

    Args:
        folder: Directory containing a *complete* GTFS feed.

    Raises:
        FileNotFoundError: If any required file is missing.

    Returns:
        Mapping ``{file_stem: DataFrame}``; e.g. ``"trips" → df``.
    """
    missing = [fname for fname in REQ_FILES if not (folder / fname).exists()]
    if missing:
