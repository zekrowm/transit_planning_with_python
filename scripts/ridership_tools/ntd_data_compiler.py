...
EXISTING_PERIOD_COLUMN_NAME = "NameOfYourExistingMonthYearColumn" # <--- IMPORTANT: UPDATE THIS
...
def read_and_prepare_ntd_file(file_path: str, sheet_name: str | None) -> pd.DataFrame | None:
    """
    ...
    print(f"Processing file: {os.path.basename(file_path)}, Sheet: {sheet_name or 'first available'}")
    ...
    try:
        ...
        if EXISTING_PERIOD_COLUMN_NAME: # Check if the global variable is set
            if EXISTING_PERIOD_COLUMN_NAME not in df.columns:
                print(f"  Warning: Specified existing period column '{EXISTING_PERIOD_COLUMN_NAME}' was NOT found in {os.path.basename(file_path)}.")
            else:
                print(f"  Info: Using existing period column '{EXISTING_PERIOD_COLUMN_NAME}' from {os.path.basename(file_path)}.")
        else:
            print(f"  Warning: 'EXISTING_PERIOD_COLUMN_NAME' not specified in config. No specific period column checked by script.")
        ...
        if DROPNA_SUBSET_ALL_NAN:
            ...
            if len(df) < original_rows:
                print(f"  Dropped {original_rows - len(df)} rows based on 'DROPNA_SUBSET_ALL_NAN': {DROPNA_SUBSET_ALL_NAN}")
        ...
        if DROPNA_SUBSET_ANY_NAN:
            ...
            if len(df) < original_rows:
                print(f"  Dropped {original_rows - len(df)} rows based on 'DROPNA_SUBSET_ANY_NAN': {DROPNA_SUBSET_ANY_NAN}")

        print(f"  Successfully processed {len(df)} rows from {os.path.basename(file_path)}.")
        return df
        ...
        print(f"ERROR reading sheet '{sheet_name}' from {os.path.basename(file_path)}: {ve}")
        return None
    except Exception as e:
        print(f"ERROR: An unexpected error occurred while reading {os.path.basename(file_path)} (Sheet: {sheet_name}): {e}")
        return None
...
def compile_ntd_data() -> pd.DataFrame | None:
    ...
    for item_index, item_details in enumerate(FILES_TO_PROCESS):
        ...
        if isinstance(item_details, tuple) and len(item_details) == 2:
            ...
            print(f"Warning: Item {item_index + 1} in 'FILES_TO_PROCESS' is not a valid (file_path, sheet_name) tuple. Skipping: {item_details}")
            continue
        
        if not file_path:
            print(f"Warning: 'file_path' is missing for item {item_index + 1}. Skipping.")
            continue
        ...
        if df_for_period is not None and not df_for_period.empty:
            ...
            print(f"  Note: File processed but resulted in an empty DataFrame: {os.path.basename(file_path)}")
    ...
    try:
        ...
        print(f"Concatenation successful. Total rows in compiled DataFrame: {len(compiled_df)}")
        ...
        if output_dir and not os.path.exists(output_dir):
            try:
                ...
                current_output_path = os.path.basename(OUTPUT_FILE_PATH) # Fallback path
                print(f"Attempting to save to current directory as: {os.path.abspath(current_output_path)}")
        ...
        if current_output_path.endswith(".csv"):
            ...
            print(f"Warning: Unknown output file extension for '{current_output_path}'. Saving as CSV to '{default_output_path}'.")
...
if __name__ == "__main__":
    ...
    if not FILES_TO_PROCESS: # Checks if the list is empty
        print("CRITICAL ERROR: 'FILES_TO_PROCESS' list is empty. Please configure input files.")
        valid_config = False
    ...
    if not EXISTING_PERIOD_COLUMN_NAME: # Checks if the string is empty
        print("CRITICAL WARNING: 'EXISTING_PERIOD_COLUMN_NAME' is not specified. "
