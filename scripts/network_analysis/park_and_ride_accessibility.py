...
CENSUS_BLOCKS_PATH = r"C:\Path\To\Your\Census_Blocks.shp"  # Replace with your file path
ACCESSIBLE_AREAS_DIR = r"C:\Path\To\Your\Output_folder"  # Replace with your folder path
OUTPUT_DIR_ANALYSIS = r"C:\Path\To\Your\Output_folder"  # Replace with your folder path
...
FACILITIES_GDF = gpd.GeoDataFrame(
    ...
    geometry=gpd.points_from_xy(
        [loc["longitude"] for loc in LOCATIONS], [loc["latitude"] for loc in LOCATIONS]
    ),
    crs="EPSG:4326",
)
...
def create_transit_network_with_transfers(
    ...
    for _, trip_group in stop_times_merged.groupby("trip_id"):
        ...
        trip_group["arrival_seconds"] = trip_group["arrival_time"].apply(
            parse_gtfs_time
        )
        trip_group["departure_seconds"] = trip_group["departure_time"].apply(
            parse_gtfs_time
        )
        stops_list = trip_group[
    ...
    for stop_id, routes_lst in stop_routes_df.items():
        for route1 in routes_lst:
            for route2 in routes_lst:
                if route1 != route2:
                    graph_obj.add_edge(
                        (stop_id, route1), (stop_id, route2), weight=xfer_time_seconds
                    )
    return graph_obj
    ...
    for origin_node in origin_nodes:
        ...
        for node, length_val in lengths_local.items():
            if length_val <= max_time_seconds:
                stop_id_val, _ = node
                if (
                    stop_id_val not in reachable_stops
                    or length_val < reachable_stops[stop_id_val]
                ):
...
def find_accessible_area_with_transfers(
    ...
    if nearby_stops.empty:
        print(
            f"No stops within {buffer_distance_miles} miles of "
    ...
    stop_routes_local = (
        FILTERED_STOP_TIMES[FILTERED_STOP_TIMES["stop_id"].isin(nearby_stop_ids)]
        .merge(FILTERED_TRIPS[["trip_id", "route_id"]], on="trip_id", how="left")
    ...
    if not reachable_stop_times:
        print(
            f"No stops reachable within {max_time_minutes} minutes from "
            f"{facility_point_gdf['name'].iloc[0]}"
        )
        return None, None
    ...
    reachable_stops_gdf["walking_distance_meters"] = reachable_stops_gdf[
        "walking_distance_meters"
    ].clip(upper=max_walking_dist)
    ...
    accessible_area_local = (
        gpd.GeoSeries([accessible_area_local], crs="EPSG:3857")
        .to_crs(epsg=4326)
        .iloc[0]
    )
...
for _, facility in FACILITIES_GDF.iterrows():
    ...
    accessible_area, reachable_stops_gdf = find_accessible_area_with_transfers(
        ...
        max_time_minutes=MAX_TRANSIT_TIME_MINUTES,
    )
    ...
    area_output_filename = os.path.join(OUTPUT_DIR, f"{safe_name}_accessible_area.shp")

    accessible_area_gdf = gpd.GeoDataFrame(
        {"name": [facility["name"]]}, geometry=[accessible_area], crs="EPSG:4326"
    )
    accessible_area_gdf.to_file(area_output_filename)
    print(f"Saved accessible area to {area_output_filename}")

    stops_output_filename = os.path.join(OUTPUT_DIR, f"{safe_name}_reachable_stops.shp")
...
for area_file in accessible_area_files:
    ...
    area_perc_sums = (
        clipped_census_gdf.groupby(UNIQUE_ID)["area_perc"].sum().reset_index()
    )
...
for _, facility in FACILITIES_GDF.iterrows():
    ...
    closest_node = min(
        road_graph.nodes, key=lambda nd: facility.geometry.distance(Point(nd))
    )
    ...
    if not lengths:
        print(
            f"No nodes reachable within {DRIVING_TIME_SECONDS / 60} minutes from "
            f"{facility_name}"
        )
        continue
    ...
    clipped_census_gdf = clipped_census_gdf.merge(
        original_areas, on=UNIQUE_ID, how="left", suffixes=("", "_orig")
    )
    clipped_census_gdf["area_perc"] = (
        clipped_census_gdf["area_ac_cl"] / clipped_census_gdf["area_ac_og"]
    )
    ...
    totals = (
        clipped_census_gdf[[f"syn_{field}" for field in DEMOGRAPHIC_FIELDS]]
        .sum()
        .round(0)
    )

    print(
        f"Demographics within {MAX_DRIVING_TIME_MINUTES}-minute drive of {facility_name}:"
    )
    ...
    print(
        f"Saved {MAX_DRIVING_TIME_MINUTES}-minute drive isochrone to {iso_output_path}"
    )
