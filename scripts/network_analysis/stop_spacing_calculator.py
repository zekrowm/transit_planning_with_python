"""
Generates segmented bus routes aligned to a road network using GTFS and shapefile inputs.

This script selects a representative bus trip from GTFS, snaps stops to a road network,
and creates route segments between stops using either direct chords or shortest paths.
Useful when GTFS shapes are missing or inaccurate.

Typical use: ArcGIS Pro workflows or Jupyter notebooks for stop spacing analysis.

Inputs:
    - GTFS folder with: stops.txt, stop_times.txt, trips.txt, calendar.txt
    - Road network shapefile
    - Bus route shapefile with route numbers
    - User-defined filters for route, direction, departure time, and service ID
    - Projected CRS for spatial operations

Outputs:
    - Unsnapped and snapped stop shapefiles
    - Chord and shortest path segments
    - Merged route line shapefiles
    - Filtered bus route and road network shapefiles
"""
...
def filter_and_select_trip(
    ...
    print(
        f"Calendar filter applied: found {len(valid_service_ids)} valid service_id(s): {valid_service_ids}"
    )
    ...
    print(
        f"Trips filtered by route, direction, calendar: {len(trips_filtered)} trips found."
    )
    ...
    print(
        f"After filtering by departure time >= {filter_departure}, {len(merged_filtered)} records remain."
    )
...
def create_unsnapped_stops(gtfs_folder, selected_trip_id, projected_crs):
    ...
    trip_stops_selected["stop_sequence"] = trip_stops_selected["stop_sequence"].astype(
        int
    )
    trip_stops_selected.sort_values("stop_sequence", inplace=True)
    print(
        "Sample selected stops (trip_id, stop_id, stop_name, stop_sequence, stop_lat, stop_lon):"
    )
    print(
        trip_stops_selected[
    ...
    unsnapped_stops_gdf = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            trip_stops_selected["stop_lon"], trip_stops_selected["stop_lat"]
        ),
        crs="EPSG:4326",
    )
...
def export_unsnapped_stops(unsnapped_stops_gdf, output_folder, selected_trip_id):
    ...
    unsnapped_stops_out = os.path.join(
        output_folder, f"unsnapped_stops_trip_{selected_trip_id}.shp"
    )
...
def export_chord_segments(
    chord_segments_gdf, output_folder, selected_trip_id, prefix="chord_segments"
):
...
def process_bus_route(
    ...
    gpd.GeoDataFrame([bus_route_feature], crs=route_shapes_gdf.crs).to_file(
        bus_route_out
    )
    ...
    bus_route_proj = gpd.GeoSeries(
        [bus_route_feature.geometry], crs=route_shapes_gdf.crs
    )
    ...
    gpd.GeoDataFrame(
        geometry=[bus_route_buffer.unary_union], crs=projected_crs
...
def snap_stops(
    unsnapped_stops_gdf, roads_for_snapping, output_folder, selected_trip_id
):
    ...
    snapped_stops_out = os.path.join(
        output_folder, f"snapped_stops_trip_{selected_trip_id}.shp"
    )
...
def merge_and_export_snapped_chord(
    snapped_chord_segments_gdf, output_folder, selected_trip_id, projected_crs
):
    """Merges snapped chord segments into a single feature and exports it."""
    merged_snapped_line = linemerge(
        [geom for geom in snapped_chord_segments_gdf.geometry]
    )
    ...
    merged_out = os.path.join(
        output_folder, f"merged_snapped_chord_trip_{selected_trip_id}.shp"
    )
...
def build_directed_network_with_stops(
    roads_gdf, snapped_stops_gdf, oneway_col="ONEWAY"
):
    """
...
def compute_and_export_shortest_paths(
    road_network, snapped_stops_gdf, output_folder, selected_trip_id, projected_crs
):
    ...
    sorted_snapped_stops = snapped_stops_gdf.sort_values("stop_sequence").reset_index(
        drop=True
    )
    ...
    for i in range(len(stop_points) - 1):
        ...
        try:
            node_path = nx.shortest_path(
                road_network, source=node_start, target=node_end, weight="weight"
            )
    ...
    if len(segments_gdf) > 0:
        ...
        if merged_line.is_empty or merged_line.geom_type == "GeometryCollection":
            ...
            merged_out = os.path.join(
                output_folder, f"merged_shortest_path_trip_{selected_trip_id}.shp"
            )
...
def main():
    ...
    unsnapped_stops_gdf = create_unsnapped_stops(
        GTFS_FOLDER, selected_trip_id, PROJECTED_CRS
    )
