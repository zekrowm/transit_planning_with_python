...
def run_ruff(files: List[str], read_only: bool) -> int:
    """Run *ruff* on *files*, write log, and return count of files with issues."""
    logger, log_fp = _setup_detailed_logger(OUTPUT_FOLDER)
    CONSOLE.info("ruff log → %s", log_fp)

    files_with_issues = 0
    ruff_base_cmd = ["ruff", "check", *RUFF_ADDITIONAL_ARGS]
    if not read_only:
    ...
    for idx, py in enumerate(files, 1):
        ...
        # Exit code 0 = clean; 1 = issues; 2+ = internal error
        if rc == 1:
            # count each reported issue line
            issues = len([l for l in (out or "").splitlines() if ":" in l])
            files_with_issues += 1
        elif rc not in (0, 1):
            # treat internal errors as one "issue"
            files_with_issues += 1
